...
const ADMIN = Deno.env.get("ADMIN") as string;
...
async function meme(conversation: MyConversation, ctx: MyContext) {
  ...
  await ctx.api.sendPhoto(
    ...
    {
      reply_markup: new InlineKeyboard().text("Approve", "approve").text(
        "Decline",
        "decline",
      ),
    },
...
bot.on("callback_query", async (ctx) => {
  ...
  switch (ctx.callbackQuery.data) {
    case "approve":
      ...
      await ctx.api.sendPhoto(
        GROUP,
        ctx.callbackQuery.message?.photo?.slice(-1)[0].file_id as string,
      );
